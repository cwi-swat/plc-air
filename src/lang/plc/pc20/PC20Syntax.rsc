module lang::plc::pc20::PC20Syntax

start syntax PC20_Assembled = AssembledInstruction+;

syntax AssembledInstruction
    = FileHeader
    | InstructionLine
    | FileEnd
    ;
...
lexical ComposedCodeBlock
    ...
    | TriggeredTimer
    | InitCall
    // End of the program, returns it back to ProgramCount N, where everything BEFORE N is only executed ONCE
    ;
...
lexical ColorName = "++" [a-zA-Z0-9\ ]* !>> [a-zA-Z0-9\ ] "++";
// * * added to remove ambiguity
lexical ConstantValue = FiveDigits;
lexical ConstantValues
    = FiveDigits
...
lexical LogicInstruction
    = AndInstruction
    | SourcePrefix ("17" | "18" | "19") WhiteSpace BitAddress NewLine
    ;
...
// 12 Fetch Constant
lexical IdentifierInstructionName
    = risingEdge: "TRIG"
    ...
    // 23 "DIVIDE"
    | end: "END"
    // 27 End of "IO" handling
    | lastInputOutput: "LSTIO"
    // 31 La"ST" Input or Output
    ;

lexical LabelInstructionName
    = subRoutineAbsoluteFalse: "JSAF"
    // 24 Jump to Subroutine, Absolute FALSE
    | subRoutineAbsoluteTrue: "JSAT"
    // 25 Jump to Subroutine, Absolute TRUE
    | backwardRelativeFalse: "JBRF"
    // 29 Jump Backwards, Relative FALSE
    | forwardRelativeFalse: "JFRF"
    // 30 Jump Forwards, Relative FALSE
    ;
