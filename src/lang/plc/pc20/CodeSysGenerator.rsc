module lang::plc::pc20::CodeSysGenerator
...
public SymbolTable systemVariables
    = [<"AlwaysOne",    "0.1", "Constant with value TRUE",                  "BOOL" >,
...
PlcProgram extractInformation(Tree plcModel, SymbolTable symbols) {
    ...
    includedLines = 0;
    initLineCount = 0;
    // Amount of lines which are only executed once
    programLines = [];
    startIfPositions = [];
    endIfPositions = [];
    ...
    if (0 != initLineCount) {
        ...
        endIfPositions += initLineCount;
    // Toss in the END_IF at the specified line
    }
    // Generic generation
    visit(plcModel) {
        case InitCall IC: {
        ...
        case IfBlock IB: {
            ...
            visit(IB) {
                ...
                case JumpSize JS: {
                    ...
                    endIfPositions += (parseInt(JS) + actualProgramCount);
                    // Notify on what position the END_IF goes
        ...
        case ComposeValue CV: {
            ...
            for (bit <- bitValues) {
                ...
                programLines
                    += "<targetData.name>.<indexOf(bitValues, bit)> := <bitData.name> ; (* <bitData.comment> *)";
            }
        ...
        case CompareConstant CC: {
            ...
            visit(CC) {
                ...
                case BitAddress BA: {
                    ...
                    programLines
                        += "<targetData.name> := (<constantValue> = <sourceData.name>) ; (* <targetData.comment> := (<constantValue> = <sourceData.comment>) *) ";
                }
            }
        ...
        case ResetBit SB: {
            ...
            visit(SB) {
                case BitAddress BA: {
                    programLines
                        += "<retrieveVariableName("<BA>", symbols)> := FALSE ; (* <retrieveComment("<BA>", symbols)> *)";
                }
                default:
                    ;
            }
        ...
        case SetBit SB: {
            ...
            visit(SB) {
                case BitAddress BA: {
                    programLines
                        += "<retrieveVariableName("<BA>", symbols)> := TRUE ; (* <retrieveComment("<BA>", symbols)> *)";
                }
                default:
                    ;
            }
        ...
        case IOJump IO: {
            ...
            programLines
                += [
                       "; (* <IO> *)",
                       "(* IO Syncing is handled by the PLC automatically, so ingore the jump *)"
                   ];
        }
        ...
        case AssignBit AB: {
            ...
            programLines
                += "(* <composeEcbPrefix("Lime", composeSourceRange(AB))>Bare EQL instruction: <trim("<AB>")> *)";
            programLines
                += "<targetInfo.name> := <evaluateExpression(lastLogic, symbols)> ; (* <targetInfo.comment> := <formatComment(lastLogic, symbols)> *)";
        }
        ...
        case AssignBitInverted AB: {
            ...
            targetInfo = retrieveInfo(bitAddress(AB), symbols);
            programLines
                += "(* <composeEcbPrefix("Lime", composeSourceRange(AB))>Bare EQLNT instruction: <trim("<AB>")> *)";
            programLines
                += "<targetInfo.name> := NOT (<evaluateExpression(lastLogic, symbols)>) ; (* <targetInfo.comment> := <formatComment(lastLogic, symbols)> *)";
        }
        ...
        case IOInstruction EL: {
            ...
            programLines = houseKeeping(EL, startIfPositions, endIfPositions, programLines);
            programLines
                += "(* <composeEcbPrefix("Lime", composeSourceRange(EL))>Bare LSTIIO instructions are used to configure the communications: <trim("<EL>")> *)";
        }
                ...
                case PartialWriteContent PWC: {
                    visit(PWC) {
                        case LogicExpression LE: {
                            writeCondition
                                = debugPrint(
                                      "Logic exp:", "IF <evaluateExpression(LE, symbols)> THEN <formatComment(LE, symbols)>"
                                  );
                        }
                        case AddressRange AR: {
                            visit(AR) {
                ...
                case PartialReadContent PRC: {
                    ...
                    visit(PRC) {
                        ...
                        case TriggerExpression TE: {
                            ...
                            totalStatement
                                = [
                                      "IF <retrieveVariableName(declaration.address, symbols)> THEN (* <declaration.comment> *)"
                                  ];
                        }
                        case AddressRange AR: {
                            visit(AR) {
                    ...
                    if (size(sourceAddresses) != size(targetAddresses)) {
                        handleError(
                            "Size of addresses do not match. Sources (<size(sourceAddresses)>): <sourceAddresses>, Targets (<size(targetAddresses)>): <targetAddresses>"
                        );
                    }
                    else {
                        debugPrint("before total");
                        debugPrint("before logic");
                        for (index <- [0..size(sourceAddresses)]) {
                            totalStatement
                                += debugPrint(
                                       "Generating Assign: ",
                                       evaluateAssign(symbols, sourceAddresses[index], targetAddresses[index])[0]
                                   );
                        }
        ...
        case AndEqual EL: {
            ...
            programLines
                += [
                       "(* <removeNewLines("<EL>")> *)",
                       "(* Target variable comment: <retrieveComment(target, symbols)> *)"
                   ];
                ...
                case BitAddressRange BAR: {
                    visit(BAR) {
                        case BitAddress BA: {
                            ...
                            programLines[size(programLines) - 2]
                                = "  Q =\> <targetInfo.name>, (* <targetInfo.comment> *) ";
                        }
                    }
                }
            }
        ...
        case PartialTrigger PT: {
            ...
            programLines += statements;
            programLines[size(programLines) - 2]
                = "  Q =\> <targetInfo.name> (* <declaration.comment> =\> <targetInfo.comment> *)";
            programLines += "  ";
        }
        ...
        case LogicCondition ECB: {
            ...
            visit(ECB) {
                case LogicExpression LE: {
                    programLines
                        += ["(* <ECB> *)", "IF <evaluateExpression(LE, symbols)> THEN <formatComment(LE, symbols)>"];
                    openCondition = true;
                }
            }
        }
    ...
    programLines
        = houseKeeping(
              actualProgramCount + 1, startIfPositions, endIfPositions, programLines, openCondition
          );
...
Statements houseKeeping(
    &T item, list[int] startIfPositions, list[int] endIfPositions, Statements currentProgram)
    = houseKeeping(item, startIfPositions, endIfPositions, currentProgram, false);
Statements houseKeeping(
    &T item, list[int] startIfPositions, list[int] endIfPositions, Statements currentProgram, bool terminateCondition) {
    currentCount = debugPrint("Insert", programCount(item));
    if (terminateCondition) {
        currentProgram = closeIf(currentProgram);
    }
    ifStarts
        = listCount(startIfPositions, endIfPositions, currentCount) +
          listCount(startIfPositions, endIfPositions, currentCount - 1);
...
Statements generateTimer(list[str] addresses, str target, SymbolTable symbols) {
    if (2 == size(addresses)) {
        ...
        return
            [
                "IF <ones> \> 0 THEN (* <retrieveComment("<ones>", symbols)> *)",
                "  <ones> := <ones> - 1 ;",
...
list[int] listCount(list[int] startValues, list[int] endValues, int itemToCount)
    = [startValues[indexOf(endValues, item)] | item <- endValues, item == itemToCount];
...
Statements closeIf(Statements programLines, str suffix) {
    ...
    for (line <- [1..programSize]) {
        if (startsWith(programLines[programSize - line], "IF ")) {
            ...
            if (contains(programLines[programSize - line], "END_IF;")) {
                count += 1;
            }
        if (count == 0) {
            break;
...
tuple[Symbol declaration, list[str] statements] evaluateTrigger(
    TriggerExpression T, SymbolTable symbols) {
...
Statements extractStatements(AssignConstant A, SymbolTable symbols) {
    ...
    debugPrint(
        "amount of addresses: <size(addresses)>, amount of constants: <lastConstantIndex + 1>"
    );
...
Statements extractStatements(&T compare, SymbolTable symbols) {
    ...
    for (index <- [0..size(sourceAddresses)]) {
        ...
        statements
            += "<prefix> (<retrieveVariableName(sourceAddresses[index], symbols)> = <retrieveVariableName(targetAddresses[index], symbols)>) (* (<retrieveComment(sourceAddresses[index], symbols)> = <retrieveComment(targetAddresses[index], symbols)>) *)";
    }
...
Statements evaluateAssign(SymbolTable symbols, str sourceAddress, str targetAddress) {
    ...
    return
        [
            "<targetName> := <sourceName> ; (* <retrieveComment(sourceAddress, symbols)> ==\>\> <retrieveComment(targetAddress, symbols)> *)"
        ];
}
...
Statements evaluateAssign(SymbolTable symbols, str address, int constantValue) {
    if (true == isBoolean(address, symbols)) {
        ...
        for (bitAddress <- retrieveAddressList(address, symbols)) {
            ...
            statements
                += "<variableName> := <toUpperCase("<getBit(constantValue, lastInteger(bitAddress))>")> ; (* <retrieveComment(variableName, symbols)> *)";
        }
        return statements;
    }
    else {
...
void generateProgram(str outputPath, PlcProgram program)
    = writeToFile(generatedFile(outputPath), generateOutput(program));
...
Variables simplifyVariables(Variables variables) {
    variableTree = innermost visit(parseVariables(variables)) {
                       case (PlcVariableList)`<PlcVariable* prev><
                           ...
                           comment
                               = parse(
                                     #DeclarationAndComment,
                                     " : INT ; (* <trim("<comment0>")> - <trim("<comment1>")> - <trim("<comment2>")> - <trim("<comment3>")> *)"
                                 );
                           insert
                           ((PlcVariableList)`<PlcVariable* prev><TextualName textual><DeclarationAndComment comment><PlcVariable* rest>`);
                       }
                       ...
                       case (PlcVariableList)`<PlcVariable *prev><
                           ...
                           while(
                               (PlcVariableList)`<UpperCaseChars name>_<Numeric number><DeclarationAndComment details><PlcVariable *newNext>`
                               := (PlcVariableList)`<PlcVariable *next>`) {
                               next = newNext;
                               actualIndex += 1;
                               nextIndex = parse(#Numeric, "<actualIndex>");
                           }
                           ...
                           stringToParse
                               = "<name>_<firstIndex>_<actualIndex> : ARRAY[<firstIndex>..<actualIndex>] OF <arrayType> ; (* Extracted aray *)";
...
Variables convertSymbols(SymbolTable symbols)
    = [extractVariable(plcSymbol) | plcSymbol <- symbols];
...
str formatName(str variableName)
    = trim(replaceAll(replaceAll(replaceAll(variableName, ",", "_"), ".", "_"), "__", "_"));
