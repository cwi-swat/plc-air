module lang::plc::pc20::Environment
...
void writeSymbolTableToFile(loc fileToSave, SymbolTable tableToSave) {
    totalSymbols = [];
    for (Symbol sneeze <- tableToSave) {
        totalSymbols += "<sneeze.name><splitter><sneeze.address><splitter><sneeze.comment>";
    }
...
str convertVariable(Variable V, SymbolTable table) {
    for (Symbol symbol <- table, trim("<V>") == symbol.name) {
...
tuple[str name, str comment] retrieveInfo(str variableName, SymbolTable table)
    = <retrieveVariableName(variableName, table), retrieveComment(variableName, table)>;
...
bool isWord(str address, SymbolTable table)
    = (true == contains(address, table)) && (false == isBoolean(address, table));
...
list[str] unnamedSymbols(SymbolTable symbols)
    = [symbol.name | symbol <- symbols, startsWith(symbol.name, "unnamed_")];
