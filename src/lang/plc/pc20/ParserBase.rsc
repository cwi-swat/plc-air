module lang::plc::pc20::ParserBase
...
// file utilities
list[sourceLine] findAmbiguousLines(str fileName, &T syntaxType)
    = [n | n <- readFile(fileName), isAmbiguous(n.text, syntaxType)];
list[sourceLine] findAmbiguousLines(loc fileName, &T syntaxType)
    = [n | n <- readFile(fileName), isAmbiguous(n.text, syntaxType)];
list[sourceLine] readFile(str fileName)
    = [<n, fileLines(fileName)[n - 1]> | n <- [0..fileSize(fileName)]];
list[sourceLine] readFile(loc fileName)
    = [<n, readFileLines(fileName)[n - 1]> | n <- [0..fileSize(fileName)]];
...
public
bool isCorrect(str textLine, &T syntaxType)
    = isParseable(textLine, syntaxType) && isUnAmbiguous(textLine, syntaxType);
