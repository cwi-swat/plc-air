module lang::plc::pc20::CodeSysChecker
...
public
void validateAndReport(PlcProgram program, str compiledFileName)
    = validateAndReport("PC20_Cycle", program, compiledFileName);
public void validateAndReport(str reportName, PlcProgram program, str compiledFileName) {
    ...
    for (line <- unexpandedlines) {
        switch(line) {
            ...
            case / Unexpanded: <sourceLine:[0-9]{5}> <programLine:[0-9]{5}> <instruction:[0-9]{2}> /: {
                ...
                errorList
                    += debugPrint(
                           "No code generated for instruction <instruction> at sourceLine <sourceLine>, programLine <programLine>"
                       );
            }
        }
    }
    ...
    int lineCount = size(program.declarations) + 4;
    // offset due to PROGRAM / VAR / END_VAR PROGRAM
            ...
            case /<startSource:[0-9]{5}>-<endSource:[0-9]{5}> is <_patternType:\w+>/: {
                ...
                if ((startCount - currentLine) != 1) {
                    if (firstLine) {
                        errorList
                            += debugPrint(
                                   "Missing program part at line <lineCount>, First <startCount - currentLine> lines of the program"
                               );
                    }
                    else if (currentLine >= startCount) {
                        errorList
                            += handleError(
                                   "Failed pattern at line <lineCount>, count is reversing. Last: <currentLine> Current start-end: <startCount>-<endCount>"
                               );
                        ...
                        errorList
                            += handleError(
                                   "Missing part of program at line <lineCount>, Programcount between <currentLine> and <startCount>"
                               );
    ...
    if (currentLine != fileRange.lastLine) {
        if (currentLine < fileRange.lastLine) {
            ...
            errorList
                += handleError(
                       "Program counter exceeds input file size. Expected: <fileRange.lastLine>, Received: <currentLine>"
                   );
        }
    }
    indentDepth = 0;
    ...
    for (line <- [0..size(program.programLines)]) {
        switch(program.programLines[line]) {
            ...
            case /^\s*IF /: {
                ...
                if ((indentDepth >= 10) && reportWarnings) {
                    warningList
                        += debugPrint(
                               "Threshold for indent depth exceeded, Possible issue with generating end_if statements around line <line>"
                           );
                }
            ...
            case /UNKNOWN_IDENTIFIER/: {
                errorList
                    += debugPrint(
                           "Found a missing address at .EXP file line <line>, Program will not compile. Verify the symbolTable generator"
                       );
            }
    ...
    if (!endIfFound && 0 < ifCount) {
        errorList
            += handleError(
                   "non-terminated IF-statement found at line <lastIf>, program will not compile"
               );
    }
    if (ifCount != endIfCount) {
        errorList
            += handleError(
                   "not all if-statements are terminated <ifCount> if-statements vs <endIfCount> end_if statements"
               );
    }
    indentError = "";
    ...
    if (!isEmpty(warningString)) {
        warningList
            += "Found <size(unknownBits)> unknown bit addresses. consider manually declaring them. (address @ .EXP line): <replaceLast(warningString, ", ", "")>";
    }
    warningString = "";
    ...
    if (!isEmpty(warningString)) {
        warningList
            += "Found <size(unknownWords)> unknown word addresses. consider manually declaring them. (address @ .EXP line): <replaceLast(warningString, ", ", "")>";
    }
    ...
    reportLines
        += debugPrint(
               "Uncovered patterns: <uncoveredPatterns>, total <uncoveredPatternLines> lines"
           );
