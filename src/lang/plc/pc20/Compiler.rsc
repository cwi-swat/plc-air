module lang::plc::pc20::Compiler

import lang::plc::pc20::FileLocations;
import IO;
import lang::plc::pc20::LabelHandler;
import List;
...
void compileToFile(str file)
    = writeToFile(generatedFile("<file>.compiled"), compile(file).compiledLines);
...
CompiledData compile(str sourceFile, str symbolTableFile)
    = compile(sourceFile, loadSymbols(symbolTableFile));
CompiledData compile(str sourceFile, SymbolTable symbols) {
    ...
    visit(generateSourceTree(sourceFile)) {
        ...
        case Instruction I: {
            debugPrint(
                "Handling <I>, line count <lineCounter>, prog count <progCounter>",
                printHandlingStatements
            );
        ...
        case PdsComment C: {
            ...
            if (lastLine == lineNumber) {
                debugPrint(
                    "Handling pds comment <C>, line count <lineCounter>, prog count <progCounter>",
                    printHandlingStatements
                );
                compiledLines += formatLine(lineCounter);
                lineCounter += 1;
                ...
                debugPrint(
                    "Skipping comment, line number <lineNumber> already occupied", printHandlingStatements
                );
            }
        }
    }
...
str replaceLabel(str compiledLine, str replacedJump)
    = padLength(substring(compiledLine, 0, 15) + replacedJump, compiledStringLength);
...
str formatLine(int lineNumber, int progCounter, int desiredLength)
    = padLength("<format(lineNumber)> <format(progCounter)> 00", desiredLength);
str formatLine(int lineNumber, int progCounter, int instruction, str address)
    = padLength(
          "<format(lineNumber)> <format(progCounter)> <format(instruction, 2)> <format(address, 5)>",
...
str format(str stringValue, int stringSize)
    = contains(stringValue, ".")
          ? right(stringValue, stringSize + 2, "0")
          : right(stringValue, stringSize, "0");
