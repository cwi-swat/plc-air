module lang::plc::pc20::Rewriter

import lang::plc::pc20::DataTypes;
import lang::plc::pc20::EcbHandler;
import ParseTree;
...
Tree rewrite(Tree tree)
    = innermost visit(tree) {
    ...
    CompiledInstruction *post>`: {
              blank
                  = parse(
                        #BlankAndNot,
                        debugPrint(
                            "BlankAndNot:", "<composeEcbPrefix("Brown", composeSourceRange(prefix, nop))>BlankAndNot"
                        )
                    );
    ...
    CompiledInstruction *post>`: {
              speed
                  = parse(
                        #QuickJumpOut,
                        debugPrint(
                            "QuickJumpOut:", "<composeEcbPrefix("Brown", composeSourceRange(prefix, jump))>QuickJumpOut"
                        )
                    );
    ...
    CompiledInstruction *post>`: {
              compareConstant
                  = parse(
                        ...
                        debugPrint(
                            "Compare Constant: ",
                            "<composeEcbPrefix("Brown", composeSourceRange(first, last))>CompareConstant <trim("<constantValue>")> = <trim("<compareValue>")> =\> <trim("<result>")>"
                        )
                    );
          ...
          case (CodeBlock) `<CompiledInstruction* pre><SkipInstruction firstNop><CompiledInstruction* post>`: {
              ...
              while(
                  (CodeBlock)`<SkipInstruction lastNop><CompiledInstruction* newPost>` :=
    ...
    CompiledInstruction* post>`: {
              ...
              bits = [bitAddress];
              while(
                  (CodeBlock)`<CompiledInstruction* newPre><SourcePrefix current>11 <BitAddress bitAddress><NewLine _>`
              ...
              composeValue
                  = parse(
                        ...
                        debugPrint(
                            "Fetch grammar:",
                            "<composeEcbPrefix("Brown", composeSourceRange(first, last))>ComposeValue <bitString>=\> <trim("<wordAddress>")> "
                        )
                    );
          ...
          case (CodeBlock)`<CompiledInstruction* pre><FetchInstruction fetch><CompiledInstruction *post>`: {
              statements = ["<fetch>"];
              while(
                  (CodeBlock)`<FetchInstruction fetch><CompiledInstruction *newPost>` :=
                  (CodeBlock)`<CompiledInstruction *post>`) {
                  post = newPost;
                  statements += ["<fetch>"];
              }
    ...
    CompiledInstruction* post>`: {
              resetBitPattern
                  = parse(
                        ...
                        debugPrint(
                            "ResetBit",
                            "<composeEcbPrefix("Brown", composeSourceRange(first, first))>ResetBit <trim("<address>")> "
                        )
                    );
    ...
    CompiledInstruction* post>`: {
              setBitPattern
                  = parse(
                        ...
                        debugPrint(
                            "SetBit",
                            "<composeEcbPrefix("Brown", composeSourceRange(first, first))>SetBit <trim("<address>")> "
                        )
                    );
          ...
          case (CodeBlock)`<CompiledInstruction* pre><IOInstruction lstio><IOInstruction endio><CompiledInstruction *post>`: {
              ...
              insert
              (CodeBlock)`<CompiledInstruction* pre><IOSynchronization ioSynchronization><CompiledInstruction *post>`;
          }
  ...
  CompiledInstruction* post>`: {
              ...
              while(
                  (CodeBlock)`<FetchConstantInstruction fetch><CompiledInstruction *newPost>` :=
                  (CodeBlock)`<CompiledInstruction *post>`) {
                  post = newPost;
                  lastFetch = fetch;
                  visit(lastFetch) {
              ...
              readConstant
                  = parse(
                        #ReadConstant,
                        debugPrint(
                            "<composeEcbPrefix("Lime", composeSourceRange(firstFetch, lastFetch))>ReadConstant <constantString>"
                        )
                    );
  ...
  CompiledInstruction *post>`: {
              ...
              try {
                  assignConstant
                      = parse(
                            #AssignConstant,
                            "<composeEcbPrefix("Lime", composeSourceRange(readConstant, writeValue))>AssignConstant <constantString> to <addressRange(writeValue)>"
                        );
                  insert
                  (CodeBlock)`<CompiledInstruction* pre><AssignConstant assignConstant><CompiledInstruction* post>`;
              }
              catch: {
          ...
          case (CodeBlock)`<CompiledInstruction* pre><StoreValue store><CompiledInstruction *post>`: {
              statements = ["<store>"];
              while(
                  (CodeBlock)`<StoreValue store><CompiledInstruction *newPost>` :=
                  (CodeBlock)`<CompiledInstruction *post>`) {
                  post = newPost;
                  statements += ["<store>"];
              }
          ...
          case (CodeBlock)`<CompiledInstruction* pre><CompareInstruction compare><CompiledInstruction *post>`: {
              statements = ["<compare>"];
              while(
                  (CodeBlock)`<CompareInstruction compare><CompiledInstruction *newPost>` :=
                  (CodeBlock)`<CompiledInstruction *post>`) {
                  post = newPost;
                  statements += ["<compare>"];
              }
    ...
    CompiledInstruction* post>`: {
              compareWithResult
                  = parse(
                        ...
                        debugPrint(
                            "CompareWithResult: ",
                            "<composeEcbPrefix("Lime", composeSourceRange(first, last))>CompareWithResult <compare> =\> <target> "
                        )
                    );
              insert
              (CodeBlock)`<CompiledInstruction* pre><CompareWithResult compareWithResult><CompiledInstruction* post>`;
          }
    ...
    CompiledInstruction *post>`: {
          ...
          case (CodeBlock) `<CompiledInstruction* pre><LogicInstruction logic><CompiledInstruction* post>`: {
              statements = [logic];
              while(
                  (CodeBlock)`<LogicInstruction logic><CompiledInstruction* newPost>` :=
                  (CodeBlock)`<CompiledInstruction* post>`) {
                  post = newPost;
                  statements += [logic];
              }
    ...
    CompiledInstruction* post>`: {
              decrementCounter
                  = parse(
                        ...
                        debugPrint(
                            "Counter",
                            "<composeEcbPrefix("Lime", composeSourceRange(first, last))>DecrementCounter <trim("<ones>")>,<trim("<tens>")> =\> <trim("<target>")> "
                        )
                    );
              insert
              ((CodeBlock)`<CompiledInstruction* pre><LogicCondition logic><DecrementCounter decrementCounter><CompiledInstruction *post>`);
          }
  	...
  	CompiledInstruction* post>`: {
              decrementCounter
                  = parse(
                        ...
                        debugPrint(
                            "Counter",
                            "<composeEcbPrefix("Lime", composeSourceRange(first, last))>DecrementCounter <trim("<ones>")>,<trim("<tens>")>,<trim("<hundreds>")>,<trim("<thousands>")> =\> <trim("<target>")> "
                        )
                    );
              insert
              ((CodeBlock)`<CompiledInstruction* pre><LogicCondition logic><DecrementCounter decrementCounter><CompiledInstruction *post>`);
    ...
    CompiledInstruction* post>`: {
              ...
              triggeredTimer
                  = parse(
                        ...
                        debugPrint(
                            "TriggeredTimer",
                            "<composeEcbPrefix("Lime", composeSourceRange(triggerBlock, last))>TriggeredTimer <triggerExp>counts <trim("<ones>")>,<trim("<tens>")>,<trim("<hundreds>")>,<trim("<thousands>")> =\> <trim("<target>")> "
                        )
                    );
          ...
          case (CodeBlock)`<CompiledInstruction *pre><LogicCondition condition><AssignBit bit><CompiledInstruction *post>`: {
              ...
              bits = [bit];
              while(
                  (CodeBlock)`<AssignBit bit><CompiledInstruction *newPost>` :=
                  (CodeBlock)`<CompiledInstruction *post>`) {
              ...
              insert
              ((CodeBlock)`<CompiledInstruction* pre><AssignBooleanExpression boolExpression><CompiledInstruction *post>`);
          }
    ...
    CompiledInstruction* post>`: {
              ...
              triggeredAssignBoolean
                  = parse(
                        ...
                        debugPrint(
                            "TriggeredAssignBoolean",
                            "<composeEcbPrefix("Brown", composeSourceRange(trigger, assign))>TriggeredAssignBoolean <triggerExp>=\> <logicExp>to <targetRange>"
                        )
                    );
              insert
              (CodeBlock)`<CompiledInstruction* pre><TriggeredAssignBoolean triggeredAssignBoolean><CompiledInstruction* post>`;
          }
    ...
    CompiledInstruction *post>`: {
              ifBlock
                  = parse(
                        ...
                        debugPrint(
                            "If Block:",
                            "<composeEcbPrefix("Brown", composeSourceRange(logic, prefix))>IfBlock <logicExpression(logic)>size <trim("<size>")>"
                        )
                    );
    ...
    CompiledInstruction* post>`: {
              ...
              partialTrigger
                  = parse(
                        ...
                        debugPrint(
                            "Partial trigger:",
                            "<composeEcbPrefix("Brown", composeSourceRange(trigger, bitTarget))>PartialTrigger <result> and <target> "
                        )
                    );
    ...
    CompiledInstruction* post>`: {
              initCall
                  = parse(
                        ...
                        debugPrint(
                            "InitCall: ",
                            "<composeEcbPrefix("Brown", composeSourceRange(prefix))>InitCall at <trim("<address>")>"
                        )
                    );
    ...
    CompiledInstruction* post>`: {
              ...
              partialRead
                  = parse(
                        ...
                        debugPrint(
                            "PartialRead:",
                            "<composeEcbPrefix("Brown", composeSourceRange(ifBlock, read))>PartialRead <readValue> by <logicExp>"
                        )
                    );
    ...
    CompiledInstruction* post>`: {
              ...
              partialRead
                  = parse(
                        ...
                        debugPrint(
                            "PartialRead:",
                            "<composeEcbPrefix("Brown", composeSourceRange(trigger, read))>PartialRead <readValue> by <triggerExp>"
                        )
                    );
              insert (CodeBlock)`<CompiledInstruction* pre><PartialRead partialRead><CompiledInstruction* post>`;
          }
    ...
    CompiledInstruction* post>`: {
              ...
              partialWrite
                  = parse(
                        ...
                        debugPrint(
                            "PartialWrite:",
                            "<composeEcbPrefix("Brown", composeSourceRange(logic, write))>PartialWrite <writeValue> by <logicExp>"
                        )
                    );
  ...
  CompiledInstruction* post>`: {
              ...
              firstRead = lastRead;
              while(
                  (CodeBlock)`<CompiledInstruction* newPre><PartialRead read>` :=
                  (CodeBlock)`<CompiledInstruction* pre>`) {
                  visit(read) {
                      case PartialReadContent PRC: {
                          reads = "<PRC>|<reads>";
              ...
              composedWrite
                  = parse(
                        ...
                        debugPrint(
                            "ComposedWrite:",
                            "<composeEcbPrefix("Brown", composeSourceRange(firstRead, write))>ComposedWrite <reads> to <partialWrite>"
                        )
                    );
...
ReadValue composeReadValue(list[str] statements)
    = parse(
          #ReadValue,
          "<composeEcbPrefix("Chocolate", statements)>ReadValue <addressRange(statements)>"
      );
WriteValue composeWriteValue(list[str] statements)
    = parse(
          #WriteValue, "<composeEcbPrefix("Lime", statements)>WriteValue <addressRange(statements)>"
      );
CompareValue composeCompareValue(list[str] statements)
    = parse(
          #CompareValue,
          "<composeEcbPrefix("LightSeaGreen", statements)>CompareValue <addressRange(statements)>"
      );
IOSynchronization composeIOSynchronization(list[str] statements)
    = parse(
          #IOSynchronization,
          debugPrint(
              "Composed IO sync: ",
              "<composeEcbPrefix("Brown", statements)>IOSynchronization <ioRange(statements)>"
          )
      );
CompareValue composeCompareValue(ReadValue readValue, CompareValue compareValue)
    = parse(
          #CompareValue,
          "<composeEcbPrefix("LightSeaGreen", composeSourceRange(readValue, compareValue))>CompareValue <addressRange(readValue, compareValue)>"
      );
AssignValue composeAssign(ReadValue readValue, WriteValue writeValue)
    = parse(
          #AssignValue,
          "<composeEcbPrefix("Lime", composeSourceRange(readValue, writeValue))>AssignValue <addressRange(readValue, writeValue)>"
      );
AndEqual composeAndEqual(SourceRange programLines, CompareValue compare, StoreBit result)
    = parse(
          #AndEqual,
          debugPrint(
              "AndEqual:",
              "<composeEcbPrefix("Tomato", programLines)>AndEqual <compareRange(compare)> =\> <bitAddress(result)> "
          )
      );
NopBlock composeNopBlock(SourceRange programLines)
    = parse(#NopBlock, "<composeEcbPrefix("LightGrey", programLines)>NopBlock");
LogicCondition composeLogicCondition(list[LogicInstruction] statements)
    = parse(
          #LogicCondition,
          "<composeEcbPrefix("Tomato", statements)>LogicCondition <formatLogic(statements)>"
      );
TriggerBlock composeTrigger(LogicCondition condition, EventInstruction trigger)
    = parse(
          #TriggerBlock,
          "<composeEcbPrefix("OliveDrab", composeSourceRange(condition, trigger))>TriggerBlock <bitAddress(trigger)> =\> <extractCondition(condition)>"
      );
BitTrigger composeBitTrigger(TriggerBlock trigger, AssignBit triggerBit)
    = parse(
          #BitTrigger,
          "<composeEcbPrefix("Cyan", composeSourceRange(trigger, triggerBit))>BitTrigger <composeBitTrigger(trigger, bitAddress(triggerBit))>"
      );
AssignBooleanExpression composeBooleanExpression(
    LogicCondition condition, list[AssignBit] bits) {
    ...
    total
        = debugPrint(
              "<trim("<composeEcbPrefix("Yellow", sourceRange)>AssignBooleanExpression <cond>")> to <bitInfo>"
          );
