module lang::plc::pc20::Interpreter
...
void FETCH_DIGIT(Address address) {
    int previous
        = shiftLeft(register.bit[register.index], 3) + shiftLeft(register.bit[register.index], 2) +
          shiftLeft(register.bit[register.index], 1) + toInt(register.bit[register.index]);
    if (1 > abs(lastFetch - programCounter)) {
        clearRegisters();
    }
...
int READ_REGISTER()
    = shiftLeft(register.bit[register.index + 3], 3) + shiftLeft(register.bit[register.index + 2], 2) +
      shiftLeft(register.bit[register.index + 1], 1) + toInt(register.bit[register.index]);
...
void handlePrint(str instruction, Address address, bool wasTrue, bool isTrue) {
    debugPrint(
        "<printHeader(instruction)> <right("<address.index>", 6, " ")>.<address.bit> was <left("<wasTrue>", 5)>, is <left("<isTrue>", 5)> condition: <condition>"
    );
}

void handlePrint(str instruction, Address address, int previous, int current) {
    debugPrint(
        "<printHeader(instruction)> <right("<address.index>", 6, " ")> was <previous>, is <current>"
    );
}
...
str printHeader(str instruction)
    = "<right("<programCounter>", 6, " ")> : <right(instruction, 6, " ")> :";
...
void handleInstruction() {
    ...
    debugPrint(
        "<programCounter> : Handling instruction <instructionNumber> with data: <address>",
        showInstructions
    );
