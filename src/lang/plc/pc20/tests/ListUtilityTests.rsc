...
test bool checkListTrimming()
    = expectEqual(["Hello", "Goodbye"], trimList(["    \r\n   \t Hello", "Goodbye \n\r\t   "]));
test bool checkListTrimmingRemoveEmptyLines()
    = expectEqual(
          ["Hello", "Goodbye"],
          trimList(["    \r\n   \t Hello", "              ", "Goodbye \n\r\t   "])
      );
...
test bool testTokenizedListTrimming()
    = expectEqual(
          ["FilledLine$Hello", "FilledLineAsWell$GoodBye"],
          trimList(["FilledLine$Hello", "EmptyLine$", "FilledLineAsWell$GoodBye"], "$")
      );

list[str] moduleNames = ["Package.rsc", "Package2.rsc"];

test bool testPadding()
    = expectEqual(
          ["import Package.rsc;", "import Package2.rsc;"], padList("import ", moduleNames, ";")
      );
...
test bool mergeLists()
    = expectEqual(
          mergedList, mergeList(firstList, secondList), "Merging should glue same indexes together"
      );

test bool testGenerating()
    = expectEqual([1, 1, 1], generateList(3, 1), "Generating a list should work");

list[str] expectedBooleanList = ["true", "true", "false", "true"];

test bool testConverting()
    = expectEqual(
          expectedBooleanList,
          convertToString([true, true, false, true]),
          "Converting to string should yield the names"
      );
