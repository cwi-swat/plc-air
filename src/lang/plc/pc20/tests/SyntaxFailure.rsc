...
// SKIP
test bool testWithOnlyConcreteSyntax()
    = expectEqual(
          expectedRange,
          composeWithoutExtraCaseLabel(sampleRead),
          "With only concrete syntax, I fail"
      );
test bool testWithDuplicateLabel()
    = expectEqual(
          expectedRange, composeWithItself(sampleRead), "Duplicate case labels will fail"
      );
test bool testWithMultipleConcrete()
    = expectEqual(
          expectedRange,
          composeWithMultipleConcrete(sampleRead),
          "Multiple concrete syntax labels labels will fail"
      );

// UNSKIP
test bool testWithConcreteAndExtraCaseLabel()
    = expectEqual(
          expectedRange,
          composeWithExtraCaseLabel(sampleRead),
          "With a sensible (unreachable) label, I will work just fine"
      );
test bool testWithAnyExtraLabel()
    = expectEqual(
          expectedRange, composeWithAnything(sampleRead), "With any other label, things also work"
      );
test bool testWithDefaultLabel()
    = expectEqual(
          expectedRange, composeWithDefault(sampleRead), "Even the default case is enough!"
      );
...
public str firstIntegerString(str inputString) {
    ...
    for (int charPosition <- [firstNumeric(inputString)..size(inputString)]) {
        ...
        if ((inLimits("0", char, "9")) || (isHex && inLimits("a", char, "f"))) {
            newString += char;
            continue;
        }
        break;
...
bool isHexaDecimal(str stringToCheck)
    = -1 == firstHex(stringToCheck) ? false : firstNumeric(stringToCheck) - 2 == firstHex(stringToCheck);
...
public
str stringToken(str stringToCheck, str firstOccurrence, str lastOccurrence)
    = stringToken(stringToCheck, firstOccurrence, findLast(stringToCheck, lastOccurrence));
public
str stringToken(str stringToCheck, int firstPosition, str lastOccurrence)
    = stringToken(stringToCheck, firstPosition, findLast(stringToCheck, lastOccurrence));
public
str stringToken(str stringToCheck, str firstOccurrence, int lastPosition)
    = stringToken(
          stringToCheck, findFirst(stringToCheck, firstOccurrence) + size(firstOccurrence), lastPosition
      );
public
str stringToken(str stringToCheck, int firstPosition, int lastPosition)
    = substring(stringToCheck, firstPosition, lastPosition);

// only provided for convenience
bool inLimits(&T lowerBound, &T actualValue, &T upperBound)
    = actualValue == limit(lowerBound, actualValue, upperBound);
&T limit(&T lowerBound, &T actualValue, &T upperBound)
    = min(max(actualValue, lowerBound), upperBound);
