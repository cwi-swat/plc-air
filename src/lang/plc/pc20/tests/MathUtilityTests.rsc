...
test bool testLimitsBelow()
    = expectEqual(0, limit(0, -10, 10), "Test value is below lower limit");
test bool testLimitsAbove()
    = expectEqual(10, limit(0, 110, 10), "Test value is above upper limit");
...
test bool testInLimitsUpperBound()
    = expectTrue(inLimits(0, 10, 10), "10 lies at upper bound 10");

test bool testLimitsString() = expectTrue(inLimits("0", "0", "1"), "0 is at lower bound");

test bool testInLimitsBelowLowerBound()
    = expectFalse(inLimits(0, -5, 10), "-5 lies below lower bound 0");
test bool testInLimitsAboveUpperBound()
    = expectFalse(inLimits(0, 15, 10), "15 lies above upper bound 10");
...
test bool testShiftingRightNothing()
    = expectEqual(8, shiftLeft(8, 0), "no positions remains the same");
test bool testShiftingLeftNothing()
    = expectEqual(8, shiftRight(8, 0), "no positions remains the same");
