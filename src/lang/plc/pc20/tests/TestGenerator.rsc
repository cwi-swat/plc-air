module lang::plc::pc20::tests::TestGenerator
...
str testMethodName(str methodLine)
    = stringToken(methodLine, "bool ", findFirst(methodLine, "()")) + "()";
...
void createTestModule(list[str] modules, list[str] testCalls) {
    ...
    addToFile(
        testModule, "module generated::MainTestModule\r\n\r\n<joinList(modules)>\r\n\r\n"
    );
    addToFile(
        testModule,
...
str createTryCatchHarness(str moduleName, str methodName)
    = "test bool try_<moduleName>_<methodName>{ debugPrint(\"Testing <moduleName>::<methodName>\"); try{ return <createTestCall(moduleName, methodName)>;} catch: { <failTestCall(moduleName, methodName)>; } return false; }";
str createTestCall(str moduleName, str methodName)
    = "checkAndReport(\"<moduleName>\",\"<methodName>\", testModule::<moduleName>::<methodName>)";
str failTestCall(str moduleName, str methodName)
    = "checkAndReport(\"<moduleName>\",\"!!! EXCEPTION IN <methodName> !!!\", false)";
