module lang::plc::pc20::tests::CodeSysTests

extend lang::plc::pc20::ParserBase;

import lang::plc::pc20::CodeSysChecker;
import lang::plc::pc20::CodeSysGenerator;
...
import lang::plc::pc20::ParserBase;
...
test bool testBooleanVariable()
    = expectEqual(
          sampleBool, extractVariable(boolean), "boolean types are distinguished based on the ."
      );
test bool testIntegerVariable()
    = expectEqual(
          sampleInt, extractVariable(integer), "int values are converted based on the missing int"
      );

Symbol faultyName = <"S_0,1SEC", "511.0", "!triggerpuls 0,1 sec.", "">;
str expectedResult = "  S_0_1SEC : BOOL ; (* 511.0 !triggerpuls 0,1 sec. *)";

test bool testInvalidChars()
    = expectEqual(
          expectedResult, extractVariable(faultyName), "Invalid chars are replaced by underscores"
      );
...
Statements expectedAssignStatements
    = [
          "(* <valueString> *)",
...
test bool testAssignValues()
    = expectEqual(
          expectedAssignStatements,
          extractStatements(assignValue, symbols),
          "Checking if assign value correcty generates the code."
      );

str constantString
    = "++Lime++CodeBlock: 00024-00038 is AssignConstant 00000 to 00320,00321,00322,00323,00324,00325,00326,00327,00328,00329,00330,00331,00332,00333";
public AssignConstant constant = parse(#AssignConstant, constantString);

public SymbolTable symbols = loadSymbols("DR_TOT_3");

list[str] expectedResetBitStatements
    = [
          "ALARM01 := FALSE ; (* !spoeldruk te hoog *)",
          "ALARM02 := FALSE ; (* !stuurlucht gestoord *)",
          "ALARM03 := FALSE ; (* !stuurspanning gestoord *)",
          "ALARM04 := FALSE ; (* !vuilwatertank bijna leeg *)"
      ];

list[str] expectedResetValidStatements
    = [
          "ALARM05 := TRUE ; (* !vuilwatertank leeg *)",
          "ALARM06 := FALSE ; (* !geleidbaarheid na spoelen *)",
          "ALARM07 := TRUE ; (* !filtercapaciteit te laag *)",
          "ALARM08 := FALSE ; (* !concentraat afvoer gestoord *)"
      ];

test bool testBitConversionWithZero()
    = expectEqual(
          expectedResetBitStatements,
          evaluateAssign(symbols, "00320", 0),
          "resetting bits word-wise must expand to bitwyse addressing"
      );
test bool testBitConversionWithValue()
    = expectEqual(
          expectedResetValidStatements,
          evaluateAssign(symbols, "00321", 5),
          "setting this value should set bit .1 and bit .3"
      );

test bool allSymbolDeclarations() {
    generateProgram(
        "AllSymbols.EXP", <convertSymbols(symbols), extractStatements(constant, symbols)>
    );
    return true;
}
...
// UNSKIP
public bool useCachedFile = false;

///< Flag bit which enables / disables the caching mechanism
...
// Simple patterns
test bool testConvertingTrigger()
    = expectEqual(
          "  TRIGGER_510_1 : R_TRIG ; (* 00123.0 - BDR0B23 *)",
          extractVariable(triggerSymbol),
          "Converting symbol with pre-filled in datatype should yield that type"
      );
...
test bool testVariable()
    = isCorrect("unreferenced_1:BOOL;(* 1.0 !0.1 sec. puls *)", #PlcVariable);
...
test bool testEmptyData() {
    ...
    return
        expectEqual(
            "UNKNOWN_IDENTIFIER",
            retrieveVariableName("00000.1", table),
            "Querying an unnamed variable yields Unknown"
        );
}
...
test bool testGeneratingBooleanData() {
    ...
    return
        expectEqual(
            "unnamed_0_1",
            retrieveVariableName("00.1", table),
            "Composing data results in data being added variable yields Unknown"
        );
}
...
test bool testGeneratingIntegerData() {
    ...
    return
        expectEqual(
            "unnamed_35",
            retrieveVariableName("035", table),
            "Composing data results in data being added variable yields Unknown"
        );
}
