module lang::plc::pc20::tests::LabelHandlerTests
...
test bool testExtractString()
    = expectEqual(expectedString, extractLabelString(testFile("simpleLabel.lbl")));
test bool testParsing() = isUnAmbiguous(expectedString, #start[LabelFile]);
test bool testReading()
    = expectEqual(labels, smallLabels(), "Reading this file should yield this list");

LabelList smallLabels() = extractLabelList(testFile("simpleLabel.lbl"));

test bool testKnownLabelLine()
    = expectEqual(4, getProgramLine(labels, "L00004"), "L00004 corresponds to program line 4");
test bool testUnknownLabelLine()
    = expectEqual(-1, getProgramLine(labels, "Non-existing"), "non-existing label returns -1");
test bool testUnlabeledLine()
    = expectEqual(
          8, getProgramLine(labels, "00008"), "label without L should be passed without replacing"
      );
