module lang::plc::pc20::tests::RewriterTests
...
str expectedBlock
    = "<expectedPrefix>AssignConstant 00000 to 00320,00321,00322,00323,00324,00325,00326,00327,00328,00329,00330,00331,00332,00333";

public LogicExpression someLogic = parse(#LogicExpression, "00220.0 OR 00220.1 ");

str sampleBooleanExpression = "++Blue++CodeBlock: 00039-00042 is AssignBooleanExpression NOT 00000.1 to 12345.1 ";
...
public AssignBooleanExpression multipleBooleanExpression
    = parse(#AssignBooleanExpression, sampleMultipleBooleanExpression);
...
public WriteValue sampleWrite
    = parse(
          #WriteValue,
          "++++CodeBlock: 00025-00038 is WriteValue 00320,00321,00322,00323,00324,00325,00326,00327,00328,00329,00330,00331,00332,00333"
      );

test bool testComposingPrefix()
    = expectEqual(
          expectedPrefix, composeEcbPrefix("Lime", composeSourceRange(sampleFetch, sampleWrite))
      );

test bool testComposingRange()
    = expectEqual(<24, 38>, composeSourceRange(sampleFetch, sampleWrite));
