module lang::plc::pc20::tests::ModelExtractorTests

import lang::plc::pc20::FileLocations;
import lang::plc::pc20::ModelExtractor;
...
bool showFigure(str fileName)
    = highLightSources(parseCompiledFile(fileName), readFileLines(compiledFile(fileName)));
...
test bool testSmallModel()
    = expectTrue(validateModelFile("first100.compiled"), "Sample of original file is valid");
test bool testInvalidModel()
    = expectFalse(
          validateModelFile("sampleIssues.compiled"),
          "File with code snippets should result in several errors"
      );

// SKIP
test bool testCompleteModel()
    = expectTrue(validateModelFile("DR_TOT_3.compiled"), "Total file check");

// UNSKIP
bool validateModelFile(str fileName)
    = examineModelCompleteness(generateFigures(parseCompiledFile(fileName), []));
