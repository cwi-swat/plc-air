@synopsis{Visualization tool for extracted statemachine models}
@pitfalls{
Because vis::Figure does not exist anymore we have to find a new
visual backend for this code. This is a big TODO.
}
module lang::plc::pc20::ModelExtractor
...
// import lang::plc::pc20::Decorator;
@synopsis{Temporary stub for Figure}
alias Figure = value;
...
bool highLightSources(Tree parseTree, list[str] sourceLines) {
    ...
    if (modelComplete) {
        debugPrint(
            "Model is complete, converted <size(sourceLines)> SLOC to <size(sourceFigures)> ECB"
        );
    }
    // TODO: from Figure
...
GraphicalModel generateFigures(str fileName)
    = generateFigures(parseCompiledFile(fileName), readFileLines(compiledFile(fileName)));
...
GraphicalBlock generateLine(str backGround, str content)
    = generateLine(backGround, "black", content);
GraphicalBlock generateLine(str backGround, str fontColor, str content)
    = <extractRange(content), generateLineFigure(backGround, fontColor, content)>;
...
int updateModelRange(int lastProgramCount, SourceRange blockToCheck) {
    ...
    if (blockToCheck.firstLine < lastProgramCount) {
        ...
        if (blockToCheck.firstLine > blockToCheck.lastLine) {
...
SourceRange extractRange(str inputSources) {
    ...
    switch(programCounterAmount) {
        ...
        default: {
            handleError(
                "Invalid amount of integers received: <programCounterAmount> in <inputSources>"
            );
            return <-1, -1>;
        }
    }
...
Tree removeBoilerPlate(Tree parseTree) {
    ...
    parseTree = innermost visit(parseTree) {
                    case (CodeBlock) `<CompiledInstruction* pre> <EmptyLine emptyLine> <CompiledInstruction* post>`: {
                        ...
                        while(
                            (CodeBlock)`<EmptyLine emptyLine><CompiledInstruction *newPost>` :=
...
ExtractionResult extractConditions(list[Statement] conditions, CompiledInstruction* post) {
    ...
    while(
        (CodeBlock)`<ConditionInstruction newC><CompiledInstruction* newPost>` :=
        (CodeBlock)`<CompiledInstruction *post>`) {
        statements += ["<newC>"];
        post = newPost;
    }
...
ExtractionResult extractConditions(CompiledInstruction* pre, ConditionInstruction condition)
    = extractConditions(pre, ["<condition>"]);
ExtractionResult extractConditions(CompiledInstruction* pre, list[Statement] conditions) {
    statements = conditions;
    while(
        (CodeBlock)`<CompiledInstruction* newPre><
...
ExtractionResult extractActions(ExecuteInstruction execute, CompiledInstruction* post)
    = extractActions(["<execute>"], post);
ExtractionResult extractActions(list[Statement] actions, CompiledInstruction* post) {
    while(
        (CodeBlock)`<ExecuteInstruction execute><
...
void constructLogic(str description) {
    ...
    try {
        ...
        handleError(
            "Unable to construct <eventDescription>: conditions: <joinList(conditions)>, actions: <joinList(actions)>"
        );
    }
}
...
CodeBlock composeBlock(CompiledInstruction* pre, CompiledInstruction* post) {
    ...
    try {
        ...
        codeBlock
            = (ComposedCodeBlock)`++++CodeBlock: <FiveDigits strFirst>-<FiveDigits strLast> is <Description description>`;
...
str generateSuffix(&T item, list[str] sourceLines)
    = "<getProgramCounter("<item>")>: <lineContent(sourceLines, getLineNumber(item))>";
