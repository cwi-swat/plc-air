module lang::plc::pc20::util::TestUtility
...
bool expectFalse(bool testMe, str messageOnFailure)
    = expectEqual(false, testMe, messageOnFailure);
...
bool expectEqual(str expected, str actual) {
    if (expected != actual) {
        iprintln(
            "Expected: --|<expected>|-- size: <size(expected)>, received --|<actual>|-- size: <size(actual)>"
        );
        return false;
    }
    return true;
...
bool expectEqualFiles(loc expectedFile, loc actualFile)
    = expectEqualFiles(expectedFile, readFileLines(actualFile));
bool expectEqualFiles(loc fileToScan, list[str] contentToCompare)
    = expectEqualFiles(readFileLines(fileToScan), contentToCompare);
bool expectEqualFiles(list[&T] fileToScan, list[&T] contentToCompare) {
    if (false == expectEqual(fileToScan, contentToCompare)) {
        addToFile(
            outputFile("test/FailedFileCompares.txt"), joinList(fileToScan - contentToCompare)
        );
        return false;
    }
    return true;
