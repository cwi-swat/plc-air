module lang::plc::pc20::util::FileUtility
...
list[loc] enumerateDirFiles(loc dir, str extension)
    = [file | file <- enumerateDirFiles(dir), endsWith(file.path, extension)];
list[loc] enumerateDirFiles(loc sampleSubDir) = enumerateDirFiles(sampleDir + sampleSubDir);
list[loc] enumerateDirFiles(loc folderLoc) {
    ...
    if (true == exists(folderLoc)) {
        try {
            ...
            handleError(
                "unable to query .ls on <folderLoc>, please add an extension to prevent this error"
            );
        }
        return locationList;
    }
...
void addToFile(loc file, list[&T] items) {
    ...
    for (line <- [0..size(items)]) {
        if (0 == line % 100) { println("<100.0 * line / size(items)>%"); }
        totalText += "<items[line]>\r\n";
    }
    addToFile(file, totalText);
}

void addToFile(loc file, str text) {
    if (exists(file)) {
