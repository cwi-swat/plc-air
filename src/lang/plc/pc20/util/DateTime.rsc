module lang::plc::pc20::util::DateTime
...
str printDate(datetime date)
    = ("<date.year>-<right("<date.month>", 2, "0")>-<right("<date.day>", 2, "0")>");

// overloads to fix issues with date incrementing
datetime incrementMonths(datetime d) = incrementMonths(d, 1);
datetime incrementMonths(datetime d, int monthsToIncrement)
    = createDate(d.year, d.month + monthsToIncrement, d.day);
