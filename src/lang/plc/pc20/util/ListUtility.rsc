module lang::plc::pc20::util::ListUtility
...
list[&T] generateList(int listSize, &T defaultValue) = [defaultValue | _ <- [0..listSize]];
...
list[str] mergeList(list[str] first, list[str] second)
    = [first[n] + second[n] | n <- [0..min(size(first), size(second))]];

list[str] padList(str prefix, list[str] lines, str suffix)
    = ["<prefix><line><suffix>" | line <- lines];
