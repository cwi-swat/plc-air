module lang::plc::pc20::EcbHandler
...
default EcbPrefix composeEcbPrefix(str colorName, list[&T] statements)
    = composeEcbPrefix(colorName, convertToString(statements));
EcbPrefix composeEcbPrefix(str colorName, list[str] statements)
    = composeEcbPrefix(
          colorName, <getProgramCounter(first(statements)), getProgramCounter(last(statements))>
      );
...
SourceLineRange composeSourceLineRange(SourceRange sourceRange)
    = parse(
          #SourceLineRange,
          "<right("<sourceRange.firstLine>", 5, "0")>-<right("<sourceRange.lastLine>", 5, "0")>"
      );
SourceRange composeSourceRange(&T firstBlock, &T secondBlock)
    = <composeSourceRange(firstBlock).firstLine, composeSourceRange(secondBlock).lastLine>;
